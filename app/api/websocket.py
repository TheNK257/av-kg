...
async def graph_ws_endpoint(websocket: WebSocket, driver, controller):
    await websocket.accept()
    print("Client connected to WebSocket")

    try:
        # task to receive commands from frontend
            ...
            try:
                while True:
                    ...
                    if action == "pause":
                        ...
                        controller.step(cmd.get("delta", 1))
            except WebSocketDisconnect:
                pass

        recv_task = asyncio.create_task(receive_commands())

        # send graph state at regular intervals
        while True:
            data = get_current_graph(driver)
            idx, total = controller.get_current_frame_index()
            data["frame_index"] = idx
            data["frame_total"] = total
            data["paused"] = controller.paused
            await websocket.send_json(data)
            await asyncio.sleep(0.5)
